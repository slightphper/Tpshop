...
class Brand extends Controller{
	...
	public function lst(){
		$data = array();
		$brandRes = Db::name('brand')->order('id desc')->paginate(2);
		$data['totalPage']   = $brandRes->lastPage();
		$brands = $brandRes->items();
		$html = '';
		$html .= '<div class="brand-list" id="recommend_brands"><ul>';
		foreach ($brands as $k => $v) {
			$html .= '<li><div class="brand-img"><a href="#" target="_blank"><img src="'.config('view_replace_str.__uploads__').'/'.$v['brand_img'].'"></a></div><div class="brand-mash"></div></li>';
		}
